...
class Registro extends CI_Controller{
            ...
            if($this->rut->valida_rut(strtoupper($this->input->post('rut')))){
                ...
                if(!$this->ion_auth->email_check($this->input->post('correo'))){
                    ...
                    if($result != FALSE){
                        ...
                        $this->notificaciones->sent_notificacion_nuevo_registro_user($additional_data,$email,$this->input->post('grupo'));
                    }else{
    ...
    public function activate($codigo = NULL){
        if($codigo){
            $decodes = base64_decode(urldecode($codigo));
            $frags = explode("|", $decodes);
            $response = $this->ion_auth_model->activate($frags[0], $frags[1]);
            if($response == 1){
